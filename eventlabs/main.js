/**
 * EVENT LABS GUYANA — HOMEPAGE JS
 * Sub-Agent B: Frontend & Multi-Page Setup Lead
 * Electric Cyan & Citron Lime geometric hero canvas, Lenis scroll, GSAP reveals, Guyana booking modal.
 */

import '../eventlabs.css'
...
// ============================================================
// HERO CANVAS — Event Labs Geometric Background (Cyan & Lime)
// ============================================================
function initHeroCanvas() {
  ...
  function initParticles() {
    particles = Array.from({ length: 70 }, () => ({
      ...
      color: Math.random() > 0.45 ? 'lime' : 'cyan'
    }))
  }
  ...
  function draw() {
    ...
    // Grid lines — Electric Cyan (rgba(54, 177, 211, 0.05))
    ctx.strokeStyle = 'rgba(54, 177, 211, 0.05)'
    ctx.lineWidth   = 0.5
    ...
    // Diagonal accent lines — Citron Lime (rgba(207, 220, 59, 0.04))
    const diagonals = [
    ...
    diagonals.forEach(d => {
      ctx.strokeStyle = 'rgba(207, 220, 59, 0.04)'
      ctx.lineWidth   = 1
      ctx.beginPath(); ctx.moveTo(d.x1, d.y1); ctx.lineTo(d.x2, d.y2); ctx.stroke()
    })

    // Particles with lime (rgba(207, 220, 59, ...)) and cyan (rgba(54, 177, 211, ...))
    particles.forEach(p => {
      p.x += p.vx; p.y += p.vy
      ...
      ctx.fillStyle = p.color === 'lime'
        ? `rgba(207, 220, 59, ${p.alpha})`
        : `rgba(54, 177, 211, ${p.alpha})`
      ctx.fill()
    })
...
function initInteractions() {
  ...
  // --- Booking Modal (Guyana Configuration) & Bento Grid ---
  initBookingModal({ country: 'GY', currency: 'GYD' })
  initBentoGrid()
}
